...
st.title("Optimization of Pipeline System (Module 1.1)")
...
def total_cost(vars):
    ...
    if include_minor:
        deltaP_minor = K_minor*(rho*v**2/2)
    else:
        deltaP_minor = 0
...
if mode == "Optimization Mode":
    ...
    result = minimize(total_cost_scaled,
                      x0=[0.3,0.75],
                      bounds=bounds,
                      method=method)
    ...
    D_opt = st.sidebar.slider("Manual Diameter (m)", 0.1, 1.0, 0.3)
    eta_opt = st.sidebar.slider("Manual Efficiency", 0.5, 0.9, 0.75)
...
for c in Ce_values:
    ...
    def temp_cost(vars):
        ...
        pipe_cost = a*D**2*L
        energy_cost = (deltaP*Q/eta)*hours*Ce_temp
        pump_cost = k*(eta**gamma_eff)
        return pipe_cost + energy_cost + pump_cost
    
    res = minimize(lambda x: temp_cost(x)/1e6,
                   x0=[0.3,0.75],
                   bounds=bounds,
                   method="L-BFGS-B")
...
st.success("Advanced Optimization Dashboard Ready!")
